...
function showAddForm(el, side) {
  ...
  f.onsubmit = async (e) => {
    ...
    indexActivities();
    await fetch('/api/activities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(state.activities) });
    state.sel[side] = id;
...
async function openWeek(startStr) {
  ...
  const days = await fetch(`/api/range?start=${week.start}&end=${end}`).then((r) => r.json());
...
async function load(dateStr) {
  ...
  const [day, recent] = await Promise.all([
    fetch('/api/day?date=' + dateStr).then((r) => r.json()),
    fetch(`/api/recent?date=${dateStr}&n=7`).then((r) => r.json()),
  ]);
...
async function init() {
  state.activities = await fetch('/api/activities').then((r) => r.json());
  indexActivities();
  ...
  $('quitBtn').onclick = async () => {
    if (!confirm('저장하고 앱을 종료할까요?')) return;
    await saveNow();                                // 종료 전에 반드시 저장
    $('bye').hidden = false;
    try { await fetch('/api/quit', { method: 'POST', body: '{}' }); } catch (_) { /* 서버가 먼저 끊길 수 있다 */ }
  };
